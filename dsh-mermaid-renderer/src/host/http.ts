...
export async function readBody(req: IncomingMessage, cap: number): Promise<string> {
  let body = ''
 ...
 */
export async function handleRender(req: IncomingMessage, res: ServerResponse, config: Config): Promise<void> {
  if (req.method !== 'POST') {
