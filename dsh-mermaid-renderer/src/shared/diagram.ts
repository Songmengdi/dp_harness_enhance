 ...
 * 决定是否做 SVG 重着色)。
 */
export function buildDarkInjection(
  source: string,
  dark: boolean,
  themeAuto: boolean,
): { diagram: string; injected: boolean } {
...
export function krokiUrlOf(baseUrl: string, krokiPath: string): string {
 ...
 */
export function parseRenderBody(raw: string, limits: RenderBodyLimits): ParseRenderBodyResult {
  ...
  const source = parsed !== null && typeof parsed === 'object'
      && typeof (parsed as { diagram_source?: unknown }).diagram_source === 'string'
    ? (parsed as { diagram_source: string }).diagram_source
    : ''
