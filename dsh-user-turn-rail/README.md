...
dsh Web GUI 会话优化插件:在对话列左缘垂直居中渲染一条**用户轮次定位导轨**——每个用户轮次一根横条,悬停时按距离渐变条长、弹出卡片预览该轮输入,点击定位并高亮对应消息;同时贡献**分档内容宽度断点**与呼吸留白(`--dsh-chat-content-width` / `--dsh-composer-side-clearance`)。
...
```sh
dsh plugin --profile <name> add https://github.com/Songmengdi/dp_harness_enhance/releases/download/dsh-user-turn-rail-v1.0.0/dsh-user-turn-rail-1.0.0.tgz
```

本地 tarball 同理:

```sh
dsh plugin --profile <name> add ./dsh-user-turn-rail-1.0.0.tgz
```
...
## 安装(备选:GitHub 源码,pin commit + 构建授权)
