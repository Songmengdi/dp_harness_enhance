...
def cmd_frames(args):
    ...
    if len(times) > 8:
        ...
        sys.exit(2)
    import re
    out_dir = re.sub(r'\.\w+ ══（以上内嵌 CLI 由 sync-cli.js 自动同步；运行时优先读 cli/dsh-vision）
...
if __name__ == '__main__':
    main()
`
  // ══ CLI_SRC_END ══（以上内嵌 CLI 由 sync-cli.js 自动同步；运行时优先读 cli/dsh-vision） ══（以上内嵌 CLI 由 sync-cli.js 自动同步；运行时优先读 cli/dsh-vision）
