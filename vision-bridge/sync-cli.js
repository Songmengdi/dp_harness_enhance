...
// 用法：node vision-bridge/sync-cli.js
...
if (!cli.includes('#!/usr/bin/env python3') || !cli.includes('dsh-vision')) {
  console.error('sync-cli: cli/dsh-vision 内容异常，拒绝同步')
  process.exit(1)
}
// String.raw 模板：仅需转义反引号与 ${（反斜杠原样保留）
const escaped = cli.replace(/`/g, '\\`').replace(/\$\{/g, '\\${')
...
if (!re.test(src)) {
  console.error('sync-cli: 未找到 CLI_SRC_BEGIN/CLI_SRC_END 标记，拒绝同步')
  process.exit(1)
}
const block = BEGIN + ' ══（由 vision-bridge/sync-cli.js 从 cli/dsh-vision 自动同步，勿手改此块）\n' +
  '  const CLI_SRC = String.raw`' + escaped + '`\n' +
  '  ' + END + ' ══（以上内嵌 CLI 由 sync-cli.js 自动同步；运行时优先读 cli/dsh-vision）'
writeFileSync(pluginPath, src.replace(re, block))
